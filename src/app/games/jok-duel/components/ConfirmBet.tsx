...
const ConfirmBet: FC<ConfirmBetProps> = ({
  currentView,
  setCurrentView,
  telegramId,
  onlinePlayers,
  ...
  const placeBet = async () => {
    try {
      ...
      // Clear the amount from localStorage after successful bet placement
      localStorage.removeItem('amount');

      // Refresh lobbies to show the new bet
      fetchLobbies();

      // Navigate to opponent selection
      setCurrentView('opponent-selection');
    } catch (error) {
      console.error('⚠️ Error placing bet:', error);
                  ...
                  <div className='flex flex-col justify-center items-center py-4'>
                    ...
                    <div className='py-4 text-center'>
                      <p className='text-[14px] font-normal'>
                        🎭 "Your bet is ready! Will an opponent dare to take up
                        the challenge?
                      </p>
                    </div>

                    <div className='bg-[#1D1D1D] h-[172px] relative rounded-xl w-full pb-1'>
                      <p className='absolute left-1/2 -translate-x-1/2 text-[16px] font-normal mt-[15px] w-full text-center'>
                        Potential Winnings
                      </p>
                      <Image
                        priority={false}
                      ...
                      <div className='absolute left-1/2 -translate-x-1/2 bottom-[5px]'>
                        <Image
                          ...
                          className=''
                        />
                        <p className='text-[20px] font-normal absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
                          +{amount * 2}
                        </p>
                      </div>
                    </div>
